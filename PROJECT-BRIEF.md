...
1. **Track 1 Phase 1 correctness PRs** (two remaining items, see "The plan" below). Approximate order — **Phase 1 items** `#6 → #7` (already shipped: #3 in PR #10, #1 resolved-delete in PR #12, #4 in PR #13, #2 in PR #14, #5 in PR #15). Each remaining PR ships with a regression test: either flipping a Phase 0 characterisation lock or adding fresh coverage.
