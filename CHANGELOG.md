...
* **TypeDoc API reference + GitHub Pages auto-publish.** New `pnpm docs:build` / `pnpm docs:watch` scripts (powered by `typedoc` devDep + `typedoc.json` config) generate `docs/api/` HTML from `src/index.ts`'s public surface — every exported class and interface gets a generated page including the JSDoc landed in PR #16 / #17. New `.github/workflows/docs.yml` runs on every push to `main` and deploys the output to `https://bitrix24.github.io/b24rabbitmq/` via the official `actions/deploy-pages` flow (no `gh-pages` branch). A new `docs` job in `.github/workflows/ci.yml` runs `pnpm docs:build` on every PR as a dry-run so broken `{@link}` references fail BEFORE merge; `treatWarningsAsErrors: true` + `validation.invalidLink/notExported: true` in the TypeDoc config make this a hard gate. README gains a Docs badge + npm downloads badge; the Documentation section leads with the API reference link. `eslint.config.mjs` ignores the generated `docs/api/**` so lint stays clean. `typedoc.json` uses `readme: "none"` + `excludeProtected: true` to keep the landing page focused on the public surface (protected subclass plumbing stays internal). **One-time maintainer setup:** Settings → Pages → Source = `GitHub Actions` (documented in CONTRIBUTING.md). (#18)
