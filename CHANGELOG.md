...
* **TypeDoc API reference + GitHub Pages auto-publish.** New `pnpm docs` script (powered by `typedoc` devDep + `typedoc.json` config) generates `docs/api/` HTML from `src/index.ts`'s public surface — every exported class and interface gets a generated page including the JSDoc landed in PR #16 / #17. New `.github/workflows/docs.yml` runs on every push to `main` and deploys the output to `https://bitrix24.github.io/b24rabbitmq/` via the official `actions/deploy-pages` flow (no `gh-pages` branch). `treatWarningsAsErrors: true` in the TypeDoc config means a broken `{@link}` or missing `entryPoint` now fails CI. README gains a Docs badge + npm downloads badge; the Documentation section leads with the API reference link. `eslint.config.mjs` ignores the generated `docs/api/**` so lint stays clean. (#18)
